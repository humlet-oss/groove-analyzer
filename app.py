...
st.write("WAVファイルをアップロードするだけで、AIが自動解析し演奏のズレと強弱を可視化します。")

st.sidebar.header("解析設定")
target_bpm = st.sidebar.number_input("正しいBPM", value=120, min_value=1)
wav_file = st.sidebar.file_uploader("WAVファイルをアップロード", type=['wav'])
...
if wav_file is not None:
    with st.spinner("AIが解析中...（1分ほどかかる場合があります）"):
        ...
        try:
            ...
            col1.metric("correct BPM", f"{target_bpm}")
